...
def build_merkle(transactions=None):
    if transactions is None: transactions = DEFAULT_TXS
    leaf_hashes = [_sha256(tx)[:16] for tx in transactions]
    txs = [MerkleTx(id=f"TX-{i}", label=tx, leaf=i) for i, tx in enumerate(transactions)]
    leaves = [MerkleLeaf(id=f"L{i}", hash=leaf_hashes[i]) for i in range(len(transactions))]
    ...
    return MerkleModel(txs=txs, leaves=leaves, branches=branches, root=root, hex="0123456789abcdef")
