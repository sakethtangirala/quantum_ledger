...
def _find_e(phi: int) -> int:
    """Smallest standard e that is coprime to phi."""
...
def compute_rsa(p: int, q: int, message: int) -> RSASandboxResponse:
    if not _is_prime(p):
    ...
    N = p * q
    if not (0 < message < N):
        raise ValueError(f"message must satisfy 0 < message < {N} (= p·q)")
    phi = (p - 1) * (q - 1)
    if phi < 3:
        raise ValueError("phi(N) too small — choose larger primes")
    e = _find_e(phi)
    d = pow(e, -1, phi)
    cipher = pow(message, e, N)
    decrypted = pow(cipher, d, N)
    return RSASandboxResponse(
        p=p, q=q, N=N, phi=phi, e=e, d=d,
        message=message, cipher=cipher, decrypted=decrypted,
    )


def build_rsa() -> RSAModel:
    p, q = 3, 5
    N = p * q                   # 15
    phi = (p - 1) * (q - 1)    # 8
    e = 7
    d = pow(e, -1, phi)         # 7  (7×7 = 49 ≡ 1 mod 8)
    message = 7
    cipher = pow(message, e, N) # 7^7 mod 15 = 13
