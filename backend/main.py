...
# Comparison data is static — mirrors data.js exactly
...
async def get_shor():
    try:
        return run_shor(15, 7).model_dump()
    except Exception as exc:
        return JSONResponse(status_code=500, content={"error": str(exc)})
...
async def get_merkle():
    try:
        return build_merkle(DEFAULT_TXS).model_dump(by_alias=True)
    except Exception as exc:
        return JSONResponse(status_code=500, content={"error": str(exc)})
...
async def sandbox_rsa(req: RSASandboxRequest):
    try:
        return compute_rsa(req.p, req.q, req.message).model_dump()
...
async def sandbox_merkle(req: MerkleSandboxRequest):
    try:
        if len(req.transactions) != 4:
            raise ValueError("exactly 4 transactions required")
...
async def sandbox_shor(req: ShorSandboxRequest):
    ...
    try:
        ...
        if N > 1000:
            raise ValueError("keep N ≤ 1000 for the classical simulation")
        if _is_prime(N):
            raise ValueError(f"{N} is prime — Shor's factors composites, try {N-1} or {N+1}")
        if not (1 < a < N):
            raise ValueError(f"a must satisfy 1 < a < {N}")
        g = math.gcd(a, N)
        if g != 1:
            raise ValueError(f"gcd({a},{N}) = {g} ≠ 1 — choose a coprime to N (or you already found a factor: {g})")
