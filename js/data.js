/* ============================================================
   The Quantum Ledger — data layer
   Attempts to load live data from the FastAPI backend at
   http://127.0.0.1:8000/api/all.  Falls back to baked-in
   static values so the experience works on Vercel without
   a running backend.  Numbers are mathematically correct.
   ============================================================ */
...
window.QL_DATA = (function () {
  ...
  const shor = {
    ...
    qftBars: (function () {
      const bars = [];
      for (let k = 0; k < 16; k++) {
        bars.push({ k, h: k % 4 === 0 ? 1 : 0.04 + Math.random() * 0.05 });
      }
      return bars;
    })(),
    steps: [
...
// Resolves (with or without error) so boot() can always proceed.
window.QL_DATA_PROMISE = (async function () {
  try {
    ...
    console.info('[QL] live data loaded from backend');
    console.group('%c QL_DATA from /api/all', 'color:#60a5fa;font-weight:bold');
    console.log('%cRSA', 'color:#a78bfa;font-weight:bold', window.QL_DATA.rsa);
    console.log('%cShor', 'color:#f87171;font-weight:bold', window.QL_DATA.shor);
    console.log('%cMerkle', 'color:#fbbf24;font-weight:bold', window.QL_DATA.merkle);
    console.log('%cCompare', 'color:#34d399;font-weight:bold', window.QL_DATA.compare);
    console.groupEnd();
  } catch (err) {
    console.info('[QL] backend unreachable — using static data (' + err.message + ')');
  }
})();
