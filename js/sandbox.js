(function() {
  ...
  function renderRSA(d) {
    ...
    showResult('rsa', `
      <div class="sb-kv-grid">
        <div class="sb-kv"><span class="sb-k">N = p\u00b7q</span><span class="sb-v">${d.p} \u00d7 ${d.q} = <b class="glow-q">${d.N}</b></span></div>
        <div class="sb-kv"><span class="sb-k">\u03c6(N)</span><span class="sb-v">(${d.p}\u22121)(${d.q}\u22121) = ${d.phi}</span></div>
        <div class="sb-kv"><span class="sb-k">e</span><span class="sb-v glow-q">${d.e} <span class="faint">\u2014 public exponent</span></span></div>
        <div class="sb-kv"><span class="sb-k">d</span><span class="sb-v glow-g">${d.d} <span class="faint">\u2014 private exponent</span></span></div>
      </div>
      <div class="sb-flow">
        <div class="sb-flow-step"><span class="sb-flow-label">plaintext</span><span class="sb-flow-val">${d.message}</span></div>
        <span class="sb-flow-arrow">m<sup>e</sup> mod N</span>
        <div class="sb-flow-step"><span class="sb-flow-label">cipher</span><span class="sb-flow-val glow-q">${d.cipher}</span></div>
        <span class="sb-flow-arrow">c<sup>d</sup> mod N</span>
        <div class="sb-flow-step"><span class="sb-flow-label">decrypted</span><span class="sb-flow-val ${ok?'glow-g':'glow-r'}">${d.decrypted} ${ok?'\u2713':'\u2717'}</span></div>
      </div>
    `);
  }
  ...
  function renderMerkle(d) {
    const leafHtml = d.leaves.map(l=>`
      <div class="sb-tnode sb-leaf"><span class="sb-nid">${l.id}</span><span class="sb-hash" data-hash="${l.hash}"></span></div>`).join('');
    const branchHtml = d.branches.map(b=>`
      <div class="sb-tnode sb-branch"><span class="sb-nid">${b.id}</span><span class="sb-hash" data-hash="${b.hash}"></span></div>`).join('');
    showResult('merkle', `
      <div class="sb-tree">
        <div class="sb-tree-row sb-root-row">
          <div class="sb-tnode sb-root"><span class="sb-nid">ROOT</span><span class="sb-hash" data-hash="${d.root.hash}"></span></div>
        </div>
