...
window.QL_shor = function (gsap, ScrollTrigger) {
  ...
  gsap.set('#shor-result', { opacity: 0, y: 24 });
  ...
  gsap.set(qubitEls, { rotateY: 0 });

  // Step highlight — uses gsap.set (instant, no competing tweens when scrubbing)
  function step(i) {
    stepEls.forEach((s, k) => gsap.set(s, {
      opacity: k === i ? 1 : (k < i ? 0.4 : 0.16),
      x: k === i ? 0 : -8,
    }));
  }

  const tl = gsap.timeline({
    scrollTrigger: { trigger: '#shor', start: 'top top', end: '+=3400', pin: true, scrub: 1.2, invalidateOnRefresh: true },
    defaults: { ease: 'power2.out' },
  });

  // ── Phase 1: machine assembles (step 01 — choose base) ──────────────────
  tl.add(() => step(0));
  ...
  tl.to(['.qm-label', '#qubits'], { opacity: 1, duration: 0.3 }, '<');

  // ── Phase 2: circuit draws itself (step 02 — superposition) ─────────────
  tl.add(() => step(1));
  ...
  // ── Phase 3: modular exp — qubits spin, core glows (step 03) ────────────
  // Spin is part of the scrub timeline — no independent loop, scrub controls it
  tl.add(() => step(2));
  tl.to('.qm-core', { boxShadow: '0 0 60px var(--quantum)', duration: 0.4 });
  tl.to(qubitEls, {
    rotateY: 900, duration: 1.4, ease: 'none',
    stagger: { each: 0.09, from: 'start' },
  }, '<0.1');

  // ── Phase 4: QFT — wave draws, bars spike at period peaks (step 04) ─────
  tl.add(() => step(3));
  tl.to('#qft', { opacity: 1, y: 0, duration: 0.4 });
  tl.to(sine, { strokeDashoffset: 0, duration: 0.6, ease: 'power1.inOut' }, '<');
  tl.to(sine, { attr: { 'stroke-opacity': 0.15 }, duration: 0.35 }, '>');
  tl.to(barEls, {
    scaleY: (i) => barEls[i].dataset.h,
    duration: 0.6, stagger: { each: 0.015, from: 'center' }, ease: 'power2.out',
  }, '<');

  // ── Phase 5: measure period — r reveals, qubits collapse (step 05) ──────
  tl.add(() => step(4));
  tl.to('#qft-r', {
    duration: 0.01,
    onStart: () => { document.getElementById('qft-r').textContent = S.period; },
    onReverseComplete: () => { document.getElementById('qft-r').textContent = '?'; },
  });
  tl.to(qubitEls, { rotateY: 0, scale: 1.5, duration: 0.15, ease: 'power4.out' });
  tl.to(qubitEls, { scale: 1, duration: 0.25, ease: 'power2.out' });

  // ── Phase 6: classical post-processing — grid tiles in (step 06) ────────
  tl.add(() => step(5));
  ...
  // ── Phase 7: extract factors → shatter → BROKEN (step 07) ───────────────
  tl.add(() => step(6));
  tl.to(primeCells, {
    scale: 1.4, color: 'var(--broken)', borderColor: 'var(--broken)',
    boxShadow: '0 0 24px var(--broken)', duration: 0.3,
  });
  tl.to(cells, {
