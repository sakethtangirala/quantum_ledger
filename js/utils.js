window.QL = (function() {
  ...
  function buildLattice(host, opts) {
    ...
    for (let i=0; i<count; i++) {
      const x=rnd(0.04,0.96)*W, y=rnd(0.05,0.95)*H, r=rnd(1.4,3.2);
      const d = el('div','q-dot');
      d.style.cssText = `position:absolute;left:${x}px;top:${y}px;width:${r*2}px;height:${r*2}px;border-radius:50%;background:var(--quantum);transform:translate(-50%,-50%);box-shadow:0 0 ${r*4}px var(--quantum);will-change:transform,opacity;`;
      host.appendChild(d);
      dots.push(d);
      pts.push({x,y,i});
    }
  ...
  function hexScramble(node, finalText, alphabet, dur, gsapRef) {
    ...
    g.to(proxy, {
      ...
      onUpdate() {
        ...
        let out='';
        for (let i=0; i<len; i++)
          out += i<settled ? finalText[i] : alphabet[(Math.random()*alphabet.length)|0];
